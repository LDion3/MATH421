...
#b
set.seed(2019)
y<-matrix(rnorm(10000),(nrows=100))
apply(y,MARGIN = 1, function(w){sum(w[w>=0])})


```
```{r}
#c
q<-apply(y,MARGIN=1,function(w){sum(w)^2})
q
hist(q,breaks=10)

#The distribution is chi-square.


```
```{r}
#d
z<-matrix(rnorm(3000),(nrows=3))
...
seven(c('three','three','three','four','five','five',NA,NA))
```
...
nine = function(a,b) {
  df1 = as_tibble(cbind(a,b))
  ggplot(df1, aes( x = df1[[1]], fill =df1[[2]]))+
           geom_bar()
           
}

nine(titanic$Survived,titanic$Embarked)
